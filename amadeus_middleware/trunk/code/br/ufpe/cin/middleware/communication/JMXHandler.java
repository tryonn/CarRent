...
import br.ufpe.cin.middleware.transport.http.HTTP;
	...
	public void initNotReliable(ObjectName configuratorName) {
		...
		try {
			...
			if (this.isServer) {
				...
				address = new UDPAddress(this.serverHost);
			}
			this.mBeanServer.registerMBean(address.createTransport(),name);
		} catch (Exception e) {
			Debug.printStack(e);
		}
	}
	...
	public void initReliable(ObjectName configuratorName) {
		...
		try {
			if (this.isServer) {
				...
				address = new TCPAddress(false);
			} else {
				name = new ObjectName(MBeansNames.CLIENT_TCP);
				address = new TCPAddress(this.serverHost,this.serverPort,this.bindPort,false);
			}
			this.mBeanServer.registerMBean(address.createTransport(),name);
		} catch (Exception e) {
			Debug.printStack(e);
	...
	public void initReliableFile(ObjectName configuratorName) {
		...
		try {
			if (this.isServer) {
				...
				address = new TCPAddress(true);
			} else {
				name = new ObjectName(MBeansNames.CLIENT_TCP_FILE);
				address = new TCPAddress(this.serverHost,this.serverPort,this.bindPort,true);
			}
			this.mBeanServer.registerMBean(address.createTransport(),name);
/*			this.mBeanServer.addNotificationListener(name,configuratorName,null,null);
			this.mBeanServer.addNotificationListener(configuratorName,name,null,null);*/
